async function ep4(){
  await T.say('その匂いは、客が階段を降りる前から店の中に届いた。','narr');
  ...
  T.sp();
  await T.say('人は、多かれ少なかれ、嘘の顔で生きている。', 'narr');
  ...
  T.sp();
  await T.npc('「私は、舞台の上で一度も、自分の感情というものを見せたことがないの」');
  ...
  await T.say('一人の女の、なんでもない朝の幸福を、ただ描いただけの映画。それなのに世界が、感嘆した。', 'narr');
  T.sp();
  ...
  await T.dlg('「賞を獲った。誰もが知る女優になった。何が、不満だ」');
  await T.npc('「あの朝が――私の、いちばん幸せな記憶に、なってしまったの」');
  T.sp();
  await T.say('女の声が、初めて、揺れた。','narr');
  T.sp();
  await T.npc('「自分の人生を、振り返るとき、いちばん、あたたかい場所を探すとき」');
  ...
  await T.say('それを捨てに来た。――嘘つきでいられなくなった嘘つきの、最後の意地か。', 'narr');
  T.sp();
  await T.say('俺は端末を引き寄せた。','narr');
  await T.dlg('「読ませてもらう。あんたのものと、借りものの境目を、確かめる」');
  ...
  await mlog('...でも、一人になると、自分が、何の役もやっていないことに気づく。');
  ...
  await T.say('この女の言うとおりだ。借りものの朝は、女の記憶に深く根を張って、本物と見分けがつかない。', 'narr');
  T.sp();
  ...
  await T.say('女は帽子を脱いで、もう化粧の崩れた顔を隠そうともしなかった。','narr');
  T.sp();
...
async function ep4EraseEnd(){
  ...
  T.sp();
  await T.say('根は深い。十年、本物の顔をして居座ってきた幸福だ。', 'narr');
...
async function ep4KeepEnd(){
  ...
  await T.dlg('「知ったからには、探せる。今度は、あんた自身の幸せな朝を」');
  T.sp();
  await T.say('女は、長いあいだ、黙っていた。','narr');
  await T.say('やがて、化粧の崩れた顔のまま、ほんの少し口角を上げた。','narr');
  await T.say('――それは、きっと台本にない、下手くそで不格好な笑みだった。','narr');
  T.sp();
  await T.npc('「……下手な顔。久しぶりに、したわ」');
  await T.dlg('「いい顔だ。それが、あんたの素顔だろう」');
  T.sp();
  await T.say('女は帽子を手に持ったまま、階段を上っていった。素顔のまま、街の灯りの中へ。','narr');
  Scene.set('IDLE');
  T.sp();
  await T.say('借りものの温度が、いつか本物の火を灯すことも、ある。', 'narr');
  await T.say('――そう信じておく。そのほうが、この商売が少しは、ましに思える。', 'narr');
}
...
async function ep4Epilogue(){
  ...
  await T.say('この冷めた手つきも、この口の悪さも。本物か、それとも、どこかで貼りつけた役柄なのか。', 'narr');
