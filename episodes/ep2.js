async function ep2(){
  ...
  await T.say('俺はタバコの煙を吐く。緑の端末光が、湿った壁をぼんやりと照らしている。','narr');
  T.sp();
  await T.say('この街では、上に行くほど、世界は綺麗になる。','narr');
  ...
  T.sp();
  await T.say('そんな男の姿を見れば、職業はだいたい分かる。','narr');
  await T.say('この男の手は、薬品でひび割れていた。だが指先だけは、妙に丁寧な動きをする。','narr');
  await T.say('――ガラスを磨く手だ。何十年も、他人の眺めを綺麗にしてきた手。', 'narr');
  T.sp();
  await T.npc('「ここで……記憶を、消してくれると」');
  await T.say('声は、低く礼儀正しかった。','narr');
  T.sp();
  ...
  await T.say('消したい相手を語る声じゃない。妙だ。', 'narr');
  T.sp();
  await T.tap();
  ...
  await T.npc('「忘れた、んじゃない。きれいに、消されてる。記憶から」');
  T.sp();
  await T.dlg('「……スクラブか」');
  ...
  await mlog('...男手ひとつで、地下の、窓のない部屋で育てた。');
  await mlog('...娘は、空を見たことがなかった。');
  ...
  await mlog('...言いつけ通り、何ひとつ、振り返らなかった。');
  await mlog('...下層のことも。窓のない部屋のことも。');
  await mlog('...私の、ことも。');
  ...
  await T.say('――こいつは、自分の手で、自分を、消した。', 'narr');
  T.sp();
  await T.tap();
  ...
  await T.say('その目は、赤く濡れていた。','narr');
  T.sp();
  await T.npc('「……消してくれ。頼む」');
  await T.npc('「あの子は、幸せなんだ。なら、私の中の、未練だけが、邪魔なんだ」');
  T.sp();
  await T.dlg('「いいか。窓は、一度ぬぐえば、それまで何が映っていたかなんて、残らない」');
  await T.dlg('「あんたの頭も、同じだ。娘も、約束も、命綱の十五年も――ぜんぶ、拭き取って、それきりだ」');
  T.sp();
...
async function ep2EraseEnd(){
  ...
  await T.say('窓のない部屋。小さな手。命綱の軋み。「いつか、空を」――十五年ぶんを塗りつぶしていく。','narr');
  T.sp();
  await T.say('キーを叩く指が、やけに冷たく感じた。', 'narr');
  await T.say('この男は、二度、娘を手放す。一度目は、上へ。二度目は、自分の頭から。', 'narr');
  await T.say('俺はそれを煙と一緒に飲み下して、削除を実行した。','narr');
  glitch(300);
  await T.sleep(350);
  ...
  T.sp();
  await T.say('男はぼんやりと周りを見回し、手持ち無沙汰に、腰の命綱の金具を指で、もてあそびはじめた。仕事前の、癖なのだろう。','narr');
  await T.npc('「……おかしいな。私は、もう、上がる時間だったか?」');
  T.sp();
  await T.say('その目が、カウンターに裏返された一枚の写真をとらえた。','narr');
  await T.say('何気なくつまみ上げ、表を返す。見覚えのない幼い女の子が、笑っていた。','narr');
  await T.npc('「……どこの子だろうな、これは。なんで、私が、こんなものを」');
  await T.say('男は首をかしげると、興味を失くしたように、写真をカウンターへ戻した。','narr');
  T.sp();
  ...
  Scene.set('IDLE');
  await T.say('カウンターの隅で、知らない女の子が、まだ笑っていた。','narr');
  T.sp();
...
async function ep2KeepEnd(){
  ...
  await T.npc('「なぜだ……! あんたは、記憶屋だろう。客の頼みを、断るのか」');
  T.sp();
  await T.say('俺は、抜いたケーブルを、ゆっくりと巻き取った。','narr');
  await T.dlg('「あんたの娘は、あんたを、忘れた」');
  await T.npc('「ここで、あんたまで、娘を忘れたら、どうなる」');
  T.sp();
  await T.say('男の指が止まった。','narr');
  T.sp();
  await T.npc('「この世のどこにも、いなくなるんだ。あんたの娘に、父親がいたって事実が」');
  ...
  T.sp();
  await T.dlg('「あんたが覚えてる限り、あんたの娘は、ちゃんと、誰かの娘だ。愛された子供だ」');
  await T.dlg('「あんたが消したら、それも、消える。最後の一つが、な」');
  await T.npc('「忘れられて、なお、忘れずにいる。それが、できるのは、もう、あんたしかいない」');
  T.sp();
  ...
  T.sp();
  await T.npc('「……明日も、あのガラスを、磨きに行くよ」');
  await T.npc('「あの子は、私を見ない。それでも、私は、あの子を見る」');
  await T.npc('「向こう側で、あの子が、笑ってるのを。それで、いいんだ」');
  T.sp();
  await T.say('男は写真を、胸の奥へしまい、戸口で深く頭を下げると、夜明け前の街へ出ていった。','narr');
  Scene.set('IDLE');
  T.sp();
...
async function ep2Epilogue(){
  ...
  await T.say('あの男は、娘を上へやるために、自分を消せと命じた。','narr');
  await T.say('――いちばん深い愛ってのは、たぶん、見返りを最初から捨ててる。','narr');
  T.sp();
  await T.say('俺にはよく分からない。覚えのある感情じゃ、ないからな。','narr');
  ...
  await T.say('――綺麗なままでいられるものなんて、この街には何ひとつ、ないんだ。', 'narr');
  T.sp();
